  ...
  async execute(interaction, config) {
    ...
    try {
      ...
      await interaction.editReply({
        content:
          "🎉 Thank you for attending the event. All channels are now open again.",
      });
      ...
      if (logChannel) {
        logChannel.send(
          `[SYSTEM] **${interaction.user.tag}** ended the event.`,
        );
      }
    } catch (error) {
      console.error("Error unlocking public channels:", error);
      await interaction.editReply({
        content: `${errorEmoji} An error occurred while unlocking channels.`,
      });
    }
  },
};
